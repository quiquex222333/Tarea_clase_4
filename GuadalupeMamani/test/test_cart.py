...
@pytest.mark.regression
def test_add_and_remove_item(page):
    """Test que verifica el flujo de añadir y luego eliminar un producto del carrito."""
    ...
    try:
        ...
        inventory.open_cart()
        assert "cart" in page.url, "No navegó a la página del carrito"

        cart.remove_first_item()
        assert inventory.items_in_cart() == 0, "El carrito debería estar vacío después de remover"

    except Exception:
        page.screenshot(path=screenshot_path("remove_item_fail"))
        raise
